...
func _init():
	logger.trace("webrtc_client._init", "")
...
func connect_to_url(url):
	logger.trace("webrtc_client.connect_to_url", url)
	close()
	code = 1000
	reason = "Unknown"
...
func close():
	logger.trace("webrtc_client.close", "")
	client.disconnect_from_host()

func _closed(was_clean = false):
	logger.trace("webrtc_client._closed", was_clean)
	emit_signal("disconnected")

func _close_request(code, reason):
	logger.trace("webrtc_client._close_request", code + "," + reason)
	self.code = code
	self.reason = reason

func _connected(protocol = ""):
	logger.trace("webrtc_client._connected", protocol)
